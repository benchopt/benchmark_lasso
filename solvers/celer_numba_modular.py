...
def cd_epoch_sparse(C, norms_X_col, X_data, X_indices, X_indptr, R, alpha, w,
    ...
    for j in C:
        ...
        w[j] = ST(w[j] + grad / norms_X_col[j] ** 2,
...
def numba_celer_primal(X, y, alpha, n_iter, p0=10, tol=1e-12, prune=True,
    ...
    # why do you call the features C
    ws = all_features.copy()  # weird init needed by numba ?
    ...
    for t in range(n_iter):
        ...
        ws_size = create_ws(prune, w, prios, p0, t, screened, ws, n_screened,
                            ws_size)
        # if ws_size === n_features then argpartition will break:
        if ws_size == n_features:
            ws = all_features
        else:
            ws = np.argpartition(np.asarray(prios), ws_size)[
        ...
        for epoch in range(max_epochs):
            ...
            if is_sparse:
                cd_epoch_sparse(ws, norms_X_col, X.data, X.indices, X.indptr, R,
                                alpha, w, n_samples)
            else:
                cd_epoch(ws, norms_X_col, X, R, alpha, w, n_samples)
