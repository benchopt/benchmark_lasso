...
SAVEFIG = False
figname = "per_iter_leukemia"

# RUN `benchopt run . --bench_config.yml` to produce the csv
BENCH_NAME = "outputs/bench_per_iteration.parquet"
...
DICT_TITLE = {
    ...
    'Lasso Regression[fit_intercept=False,reg=0.05]': (
        r'$\lambda = 0.05 \lambda_{\mathrm{max}}$'
    ),
    'Lasso Regression[fit_intercept=False,reg=0.5]': (
        r'$\lambda = 0.5 \lambda_{\mathrm{max}}$'
    ),
...
for idx_data, dataset in enumerate(datasets):
    ...
    for idx_obj, objective in enumerate(objectives):
        ...
        for i, solver_name in enumerate(solvers):
            ...
            y = df3["objective_value"] - c_star

            linestyle = '-'
            if solver_name in ("snapml[gpu=True]", "cuml[qn]", "cuml[cd]"):
                linestyle = '--'
            ax.loglog(
                df3.stop_val, y, color=style[solver_name][0],
        ...
        axarr[len(datasets)-1, idx_obj].set_xlabel(
            "number of iterations", fontsize=labelsize
        )
